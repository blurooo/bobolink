...
function Bobolink(options) {
  ...
  schedulingModes[constants.SCHEDULE_MODE_FREQUENCY] = {
    ...
    init: () => {
      // 与1000ms求公约数
      let gcd = utils.getGCD(1000, queueOptions.countPerSecond);
      // 得到以毫秒为单位的一个调度频率
  ...
  function discardOldest(len) {
    ...
    if (queueOptions.newPrior) {
      this.task.length = this.task.length - len;
    } else {
      // 否则队头为新任务
      this.task.splice(0, len);
    }
  }

  this.put = this.push = function(datas, prior) {
    // 数据不合法
    if (!datas || (!Array.isArray(datas) && !(datas instanceof Function))) {
      return Promise.reject(constants.INVALID);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(datas)) {
        ...
        if (queueOptions.taskMode === constants.TASK_MODE_FUNCTION && !(datas instanceof Function)) {
          return Promise.reject(constants.TASK_ERROR);
        }
        putTask({
          resolve,
          reject,
  ...
  function taskHandler(task, p) {
    ...
    // 组任务第一次执行时标记开始时间
    !taskTag[task.tag].startTime && (taskTag[task.tag].startTime = startTime);
    p.then(res => {
      self.runningTasksCount--;
...
// 挂载常量
Object.assign(Bobolink, constants);
