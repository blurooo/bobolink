...
function Bobolink(options) {
  ...
  schedulingModes[constants.SCHEDULE_MODE_FREQUENCY] = {
    ...
    init: () => {
      ...
      // 得到每个调度频率内需要调度的任务数
      let taskNumber = queueOptions.countPerSecond / gcd;
      // 每个调度频率调度期望的任务数
  ...
  function discardOldest(len) {
    ...
    if (queueOptions.newPrior) {
      queueTasks.length = queueTasks.length - len;
    } else {
      // 否则队头为新任务
      let oldestTasks = queueTasks.splice(0, len);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (Array.isArray(datas)) {
        ...
        let validData = datas.filter(task => {
          ...
          if (queueOptions.taskMode === constants.TASK_MODE_DATA) {
            return !(task instanceof Function);
          }
          return true;
        });
